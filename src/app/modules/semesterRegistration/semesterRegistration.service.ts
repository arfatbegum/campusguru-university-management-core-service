...
const enrollIntoCourse = async (
  ...
  const result = studentSemesterRegistrationCourseService.enrollIntoCourse(
    authUserId,
    payload
  );
  if (result) {
...
const withdrewFromCourse = async (
  ...
  const result = studentSemesterRegistrationCourseService.withdrewFromCourse(
    authUserId,
    payload
  );
  if (result) {
...
const startNewSemester = async (
  ...
  if (semesterDetails) {
    await RedisClient.publish(
      EVENT_NEW_SEMESTER_STARTED,
      JSON.stringify(semesterDetails)
    );
  }
  return {
...
const getMySemesterRegisterCourses = async (authUserId: string) => {
